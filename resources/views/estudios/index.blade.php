<x-app-layout>
    <x-slot name="header">
        ...
        <x-nav-link :href="route('estudios.create', $trabajadore->id)">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('+ Nuevo Estudio') }}</h2>
        </x-nav-link>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-1 text-gray-900 md:flex md:justify-center text-2xl">
                    <h1><strong>Estudios del personal: </strong> {{ $trabajadore->apellidos }}, {{ $trabajadore->nombres }}</h1>
                </div>
                ...
                <div class="md:flex md:justify-center p-5">
                    <ul class="divide-y divide-gray-200 w-3/4">
                        @forelse ($trabajadore->estudio as $estudi)
                            <li class="p-0 flex items-center">
                                <div class="flex-1">
                                        <p class="text-xl font-medium text-gray-800">{{$estudi->centroestudios}}</p>
                                        <p class="text-sm text-gray-600">{{$estudi->tipoestudios}}</p>
                                </div>
                                ...
                                <div>
                                    <a
                                        ...
                                        href="{{ asset('storage/archivoestudio/' . $estudi->archivoestudios) }}"
                                        target="_blank"
                        ...
                        @empty
                            <p class="p-3 text-center text-sm text-gray-600">No hay ningun estudio aun</p>
                        @endforelse
                    </ul>
                </div>
